...
export const storageHelpers = {
    ...
    set: (key, value) => {
        try {
            ...
            if (key.length > 500) {
                console.warn(
                    `⚠️ localStorage key is very long (${key.length} chars):`,
                    key.substring(0, 100) + '...'
                );
            ...
            if (serialized.length > 5 * 1024 * 1024) {
                console.error(
                    `❌ Value too large for localStorage (${(
                        serialized.length /
                        1024 /
                        1024
                    ).toFixed(2)}MB)`
            ...
            if (error.name === 'QuotaExceededError' || error.code === 22) {
                console.error(
                    `❌ localStorage quota exceeded for key: ${key.substring(
                        0,
                        100
                    )}`
                );
                ...
                console.error(
                    `❌ localStorage security error (private browsing?): ${key.substring(
                        0,
                        100
                    )}`
                );
                ...
                console.warn(
                    `⚠️ Failed to save ${key.substring(
                        0,
                        100
                    )} to localStorage:`,
                    error
...
export function migrateAndCleanupLocalStorage() {
    ...
    storyCacheKeys.forEach(key => {
        try {
            ...
            if (key.length > 200) {
                console.log(
                    `🧹 [MIGRATION] Removing old-style long cache key (${key.length} chars):`,
                    key.substring(0, 100) + '...'
                );
                storageHelpers.remove(key);
                invalidKeys++;
            ...
            if (cached?.timestamp) {
                ...
                console.log(
                    `🧹 [MIGRATION] Removing invalid cache key:`,
                    key.substring(0, 100)
                );
                storageHelpers.remove(key);
                invalidKeys++;
            ...
            console.warn(
                `⚠️ [MIGRATION] Error processing cache key ${key.substring(
                    0,
                    100
                )}, removing:`,
                error
            ...
            try {
                ...
                console.error(
                    `❌ [MIGRATION] Failed to remove invalid key:`,
                    removeError
                );
            }
        }
