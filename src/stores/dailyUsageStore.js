...
import { getDailyLimitForUser, isLimitReached, getRemainingGenerations } from '../config/limits.js';
...
export async function initializeDailyUsage() {
    try {
        ...
        if (!usageData) {
            ...
            if (usageData) {
                console.log('✅ Daily usage loaded from localStorage:', usageData);
            }
        }
    ...
    try {
        ...
        if (loggedIn && account?.userId) {
            ...
            saveUsageToAPI(account, usageData).catch(error => {
                console.warn('⚠️ Failed to save usage to API (background):', error);
                // Rollback on error?
            });
        }
...
async function loadUsageFromAPI(account) {
    try {
        ...
        if (result.success && result.account) {
            // Extract usage from metadata.dailyUsage
            const dailyUsage = result.account.metadata?.dailyUsage;
...
export const isLimitReachedStore = derived(
    dailyLimit,
    $dailyLimit => isLimitReached($dailyLimit.used, $dailyLimit.limit)
);

// Get remaining generations
export const remainingGenerations = derived(
    dailyLimit,
    $dailyLimit => getRemainingGenerations($dailyLimit.used, $dailyLimit.limit)
);
