<script>
    ...
    function getMessageType(msg) {
      ...
      if (lowerMsg.includes('pending') || lowerMsg.includes('sending')) return 'contact';
    ...
    modalMessage.subscribe(value => {
      if (value) {
        ...
        // Auto-close after duration (except for errors which require manual close)
        if (getMessageType(value) !== 'error') {
...
{#if showMessage}
  <FocusManager active={showMessage} restoreFocus={true}>
      ...
      >
        ...
        >
          <!-- Show different image based on the message type -->
          {#if messageType === 'contact' || messageType === 'success'}
            <img
          ...
          >
            ...
            {#if messageType === 'error'}
              <button 
                class="mt-4 px-4 py-2 text-black bg-white rounded-full hover:bg-gray-200 transition-colors duration-200"
