 ...
 */
export async function testAPIDirectly(): Promise<TestResult | null> {
    console.clear();
    console.log('═══════════════════════════════════════════════════════');
    ...
    try {
        ...
        const data = await response.json() as { success: boolean; account?: Account };
        ...
        console.log('Length:', (metadata.usageHistory as unknown[] | undefined)?.length || 0);
        console.log('');
        ...
        const usageHistory = metadata.usageHistory as Array<{ date: string; used: number }>;
        ...
        console.log('  Entries:', usageHistory.length);
        console.log('  First entry:', usageHistory[0]);
        console.log(
            '  Last entry:',
        ...
        const lastDate = usageHistory[usageHistory.length - 1]?.date;
        console.log('  Date range:', lastDate, 'to', firstDate);

        const nonZero = usageHistory.filter(e => e.used > 0);
        ...
        const avgUsed = totalUsed / usageHistory.length;
        ...
        console.log('  ✅', usageHistory.length, 'entries available');
        console.log('  ✅ Date range:', lastDate, 'to', firstDate);
            ...
            account: {
                ...
                profile:
                    typeof account.profile === 'string'
                        ? JSON.parse(account.profile) as Account['profile']
                        : account.profile
            }
        };
    } catch (error) {
...
if (typeof window !== 'undefined') {
    (window as { testAPIDirectly?: typeof testAPIDirectly }).testAPIDirectly = testAPIDirectly;
