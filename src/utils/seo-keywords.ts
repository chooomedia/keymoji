...
export type PageType = 'home' | 'versions' | 'contact' | 'blog';
...
export interface SEOScore {
    ...
    keywordDensity: Record<string, KeywordDensity>;
}

// Primäre Keywords für Hauptseiten
export const PRIMARY_KEYWORDS: Record<PageType, string[]> = {
    home: [
        'emoji password',
        'password generator',
        'secure passwords',
...
// LSI (Latent Semantic Indexing) Keywords
export const LSI_KEYWORDS: Record<PageType, string[]> = {
    home: [
        'unicode passwords',
        'symbol passwords',
        'visual passwords',
...
// Long-tail Keywords für bessere Rankings
export const LONG_TAIL_KEYWORDS: Record<PageType, string[]> = {
    home: [
...
export const TECHNICAL_KEYWORDS: Partial<Record<PageType, string[]>> = {
    home: [
...
export const LOCALIZED_KEYWORDS: Record<Language, Partial<Record<PageType, string[]>>> = {
    de: {
        home: [
            'emoji passwort',
            'passwort generator',
...
export const META_KEYWORDS: Record<PageType, string> = {
...
export function getMetaKeywords(pageType: PageType): string {
    return META_KEYWORDS[pageType] || META_KEYWORDS.home;
}

export function generateKeywordString(
    pageType: PageType,
    language: Language = 'en'
): string {
...
export function analyzeKeywordDensity(
    text: string,
    keywords: string[]
): Record<string, KeywordDensity> {
    const wordCount = text.toLowerCase().split(/\s+/).length;
    const density: Record<string, KeywordDensity> = {};
...
export function calculateSEOScore(
    ...
    const averageDensity =
        keywords.all.length > 0 ? totalDensity / keywords.all.length : 0;
