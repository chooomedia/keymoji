// src/utils/chartDebugger.js
// Comprehensive Chart Data Flow Debugger

import { get } from 'svelte/store';
import { currentAccount, isLoggedIn, accountTier } from '../stores/appStores.js';
...
export function fullDiagnosis() {
    ...
    if (typeof account.metadata === 'string') {
        ...
        console.log('Raw metadata preview:', account.metadata.substring(0, 100) + '...');
        console.log('💡 Solution: safeJSONParse() may not be working');
        console.log('💡 Try manual parse:');
        console.log('   window.$currentAccount.metadata = JSON.parse(window.$currentAccount.metadata)');
        return;
    }
    ...
    console.log('UsageHistory length:', account.metadata?.usageHistory?.length || 0);
    ...
    if (!account.metadata?.usageHistory) {
        ...
        console.log('💡 Solution: Update Google Sheets metadata with usageHistory array');
    ...
    if (account.metadata.usageHistory.length === 0) {
        ...
        console.log('💡 Solution: Add data to Google Sheets metadata.usageHistory');
        return;
    }

    console.log('✅ UsageHistory is a valid array with', account.metadata.usageHistory.length, 'entries');
    console.log('');
    ...
    console.log('Date range:', `${lastEntry?.date || 'unknown'} to ${firstEntry?.date || 'unknown'}`);
    ...
    if (!hasValidStructure) {
        ...
        const invalidEntries = history.filter(e => 
    ...
    console.log('Points with matching history:', chartData.filter(d => d.found).length);
    ...
    if (chartData.filter(d => d.value > 0).length === 0) {
        ...
        console.log('');
        console.log('History date range:', `${lastEntry.date} to ${firstEntry.date}`);
        console.log('Chart expecting:', `${chartData[0].date} to ${chartData[chartData.length - 1].date}`);
        return;
    }
    ...
    console.log('  • Chart Data: ✅', chartData.filter(d => d.value > 0).length, 'points');
    console.log('');
...
export function quickCheck() {
    ...
    const allGood = Object.values(checks).every(v => v === true || v > 0);
 ...
 */
export function forceParseMetadata() {
    const account = get(currentAccount);
...
export function injectTestData() {
    ...
    account.metadata.usageHistory = usageHistory;
    currentAccount.set(account);
