 ...
 */
export function instantChartTest(): boolean {
    console.clear();
    console.log('═══════════════════════════════════════════════════════');
    ...
    if (!Array.isArray(metadata.usageHistory)) {
        ...
        console.log('   Current type:', typeof metadata.usageHistory);
    ...
    console.log('─────────────────────────');
    const history = metadata.usageHistory as UsageHistoryEntry[];
 ...
 */
export function showChartData(): UsageHistoryEntry[] {
    const account = currentAccount;
    const metadata = account?.metadata as Record<string, unknown> | undefined;
    const history = (metadata?.usageHistory as UsageHistoryEntry[] | undefined) || [];
 ...
 */
export function verifyChartGeneration(): Array<{ date: string; value: number; found: boolean; source: string }> {
    const account = currentAccount;
    const metadata = account?.metadata as Record<string, unknown> | undefined;
    const history = (metadata?.usageHistory as UsageHistoryEntry[] | undefined) || [];
    ...
    const chartData: Array<{ date: string; value: number; found: boolean; source: string }> = [];
...
if (typeof window !== 'undefined') {
    (window as { instantChartTest?: typeof instantChartTest }).instantChartTest = instantChartTest;
    (window as { showChartData?: typeof showChartData }).showChartData = showChartData;
    (window as { verifyChartGeneration?: typeof verifyChartGeneration }).verifyChartGeneration = verifyChartGeneration;
