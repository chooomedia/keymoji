...
} from '../stores/dailyUsageStore';
import { storageHelpers, STORAGE_KEYS } from '../config/storage';
import { getDailyLimitForUser } from '../config/limits';

interface DebugResult {
    dailyLimit: { limit: number; used: number };
    isLoggedIn: boolean;
    accountTier: 'free' | 'pro';
    isLimitReached: boolean;
    remaining: number;
    status: typeof usageStatus;
    account: {
        userId?: string;
        email?: string;
        dailyUsage?: unknown;
 ...
 */
export function debugDailyUsage(): DebugResult {
    const limit = dailyLimit;
    const logged = isLoggedIn;
    const tier = accountTier;
    const account = currentAccount;
    const status = usageStatus;
    const isReached = isLimitReachedStore;
 ...
 */
export async function testIncrementUsage(): Promise<void> {
 ...
 */
export async function testResetUsage(): Promise<void> {
 ...
 */
export async function testReInitialize(): Promise<void> {
 ...
 */
export async function testReachLimit(): Promise<void> {
    ...
    for (let i = 0; i < increments; i++) {
        try {
            await incrementDailyUsage();
            console.log(`Increment ${i + 1}/${increments}:`, dailyLimit);
        } catch (error) {
...
export function checkConsistency(): string[] {
    ...
    const storeData = dailyLimit;
    const localData = storageHelpers.get(STORAGE_KEYS.DAILY_USAGE) as { used?: number; limit?: number } | null;
...
export default keymojiDailyUsageDebug;
